...
def init_db():
    ...
    try:
        ...
        if settings.FALLBACK_TO_SQLITE:
            logger.info(f"Falling back to SQLite database at: {settings.SQLITE_URL}")
            engine = create_engine(settings.SQLITE_URL, connect_args={"check_same_thread": False}, echo=True)
        else:
            raise e
